...
initial begin
    ...
    if (MAX_SIM_CYCLES > 0) begin
        // ISA test mode: run for N cycles, dump registers, then stop
        ...
        $display("x03 = 0x%h (Expected: 0xffffffd6)", top_ins.decode_stage_inst.regfile.registers[3]);
        $display("x04 = 0x%h (Expected: 0xffffffff)", top_ins.decode_stage_inst.regfile.registers[4]);
        $display("x05 = 0x%h (Expected: 0xffffffff)", top_ins.decode_stage_inst.regfile.registers[5]);
        $display("x06 = 0x%h (Expected: 0x00000005)", top_ins.decode_stage_inst.regfile.registers[6]);
        $display("x07 = 0x%h (Expected: 0xfffffff8)", top_ins.decode_stage_inst.regfile.registers[7]);
        $display("x08 = 0x%h (Expected: 0x0000000e)", top_ins.decode_stage_inst.regfile.registers[8]);
        $display("x09 = 0x%h (Expected: 0xfffffffe)", top_ins.decode_stage_inst.regfile.registers[9]);
        ...
        $display("x15 = 0x%h (Expected: 0x00000002)", top_ins.decode_stage_inst.regfile.registers[15]);
        $display("x16 = 0x%h (Expected: 0xffffffff)", top_ins.decode_stage_inst.regfile.registers[16]);
        $display("x17 = 0x%h (Expected: 0x12345678)", top_ins.decode_stage_inst.regfile.registers[17]);
    ...
    // When MAX_SIM_CYCLES == 0, simulation runs forever (Snake game mode).
    // Stop manually with Ctrl+C in the simulator terminal.
end

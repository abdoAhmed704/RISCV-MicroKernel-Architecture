module riscv_top_pipeline(input clk, input rst_n, output reg [31:0] result);
...
riscv_fetch_stage new_fet(
    .clk(clk), .rst_n(rst_n), .PCTargetE(PCTargetE), .PCSrcE(PCSrcE), .instrD(instrD),
...
riscv_execute_stage execute_stage_inst(
    .clk(clk), .rst_n(rst_n), .PCE(PCE), .PCPlus4E(PCPlus4E), .RegWriteE(RegWriteE), .ResultSrcE(ResultSrcE), 

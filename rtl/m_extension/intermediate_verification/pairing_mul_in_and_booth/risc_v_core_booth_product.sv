...
logic [XLEN-1 : 0]         CS_multiplicand , NS_multiplicand;
...
always_ff @( posedge i_booth_clk , negedge i_booth_rstn ) begin 
    if (!(i_booth_rstn)) 
        begin
            state_reg        <= IDLE;
            CS_accum         <= 0;
            CS_multiplier    <= i_booth_multiplier;
            CS_Q_n_minus_one <= 0;
            CS_multiplicand  <= i_booth_multiplicand;
            CS_counter       <= XLEN;
        ...
        begin
            ...
            CS_multiplicand  <= NS_multiplicand;
...
// i did it through the next 5 lines of codes
    ...
    NS_multiplicand  = CS_multiplicand;
    ...
    case(state_reg)
        IDLE:
            begin
                ...
                o_booth_done     = 0;
                o_booth_product  = 0;
                NS_multiplicand  = i_booth_multiplicand;
        ...
        MUL:
            begin
                ...
                2'b00 :
                    begin
                        NS_Q_n_minus_one = NS_multiplier[0];
                        NS_multiplier    = {NS_accum[0]      , NS_multiplier[XLEN-1 : 1]};
                        NS_accum         = {NS_accum[XLEN-1] , NS_accum[XLEN-1 : 1]};
                    end

                2'b01 :
                    begin
                        NS_accum = CS_accum + NS_multiplicand;
                        NS_Q_n_minus_one = NS_multiplier[0];
                        NS_multiplier    = {NS_accum[0]      , NS_multiplier[XLEN-1 : 1]};
                        NS_accum         = {NS_accum[XLEN-1] , NS_accum[XLEN-1 : 1]}; 
                    end
                ...
                2'b10 :
                    begin
                        NS_accum = CS_accum + ~NS_multiplicand + 1;
                        NS_Q_n_minus_one = NS_multiplier[0];
                        NS_multiplier    = {NS_accum[0]      , NS_multiplier[XLEN-1 : 1]};
                        NS_accum         = {NS_accum[XLEN-1] , NS_accum[XLEN-1 : 1]}; 
                    end
                ...
                if(NS_counter == 0)
                    begin
                        o_booth_done    = 1;
                        o_booth_product = {NS_accum  , NS_multiplier}; 
                        nxt_state       = IDLE ;
                    end
                else 
                    begin
